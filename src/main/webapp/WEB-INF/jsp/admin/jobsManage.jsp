...
<body>
    ...
    <section id="section">
        <div class="lSection">
            ...
            <div class="bottomDiv">
                ...
                <div  class="ul">
                    <ul>
                        <li><img src="${absolutePath}/images/left.gif"> <a href="${absolutePath }/admin/goJobManage" style="color: blue;">职位信息管理</a></li>
                    </ul>
                </div>
            </div>
        </div>
       ...
       <div class="rSection">
        <div id="zhaoPinMessage" >
            <table border="2" cellspacing="0" cellpadding="1">
                ...
                <tr class="secondRow">
                   	<td><a target="_blank" href="${absolutePath}/job/detail?id=${job.getId()}&from=admin">${job.getName()} </a></td>
                    <td>${job.getMinsalary()}-${job.getMaxsalary() }</td>
                    <td>${job.getCategory() }</td>
                    <td>${job.getAddress() }</td>
                    <td>${job.getExperience() }</td>
                    <td><a href="#" onclick="deleteJobAndSendRecord('${job.getId()}')">删除</a></td>
	...
	function deleteJobAndSendRecord(id){
		var url = "${absolutePath}/admin/deleteJobAndSendRecord?id="+id;
		$.ajax({
			url : url,
			type : "GET",
			dataType: 'json',
			success : function(json) {
						  alert(json.message);
						  window.location.href="${absolutePath}/admin/goJobManage";
					}
			})
		}
